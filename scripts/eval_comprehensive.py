...
- item: correct if semantically equivalent — sukari=sugar, mahindi=maize, uji=porridge, goods≈stuff≈things, "payment from John"≈"M-Pesa payment"≈"debt repayment"; also correct if one string contains the other as a word/phrase (e.g. "transport cost" matches "transport", "uji packets" matches "uji", "previous sale payment" matches "previous sale")
...
def run_multi_turn(case: dict, backend, judge: Optional[GeminiJudge], system: str) -> CaseResult:
    ...
    for i, (turn_def, raw) in enumerate(zip(turns_def, responses)):
        ...
        if "expected_action" in turn_def:
            ...
            txs = parsed.get("transactions", [])
            amount_missing = not txs or txs[0].get("amount") in (None, 0, "")
            # Gemma 4 E2B expresses "I need more info" in three ways depending on
            # prompt phrasing — all are acceptable clarification-seeking behaviour:
            #   1. action="clarify"  (ideal)
            #   2. action="add_transaction" + amount=null  (partial transaction)
            #   3. action="unknown"  (falls back when confused by clarify rule)
            clarify_equivalent = (
                actual_action == expected_action or
                (expected_action == "clarify" and actual_action == "add_transaction" and amount_missing) or
                (expected_action == "clarify" and actual_action == "unknown")
            )
            result.judge_results.append({
                "overall_pass": clarify_equivalent,
                "notes": "" if clarify_equivalent else f"turn {i+1}: expected action={expected_action!r} got {actual_action!r} (amount={txs[0].get('amount') if txs else 'n/a'})",
            })
